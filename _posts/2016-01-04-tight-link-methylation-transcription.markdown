...
A ChIP-Seq experiment validated the increased binding of Nrf1 in regions that were methylated in wildtype cells before, and a matched RNA-Seq experiment revealed elevated expression at loci that were newly bound by Nrf1. 
...
Transferring cells back to the original medium enabled the authors to test whether newly bound DHSs prevents from remethylation of those sites.
...
In summary, *settler* transcription factors that are restricted to bind DNA through DNA methylation depends on indirect cooperation with specific factors, as well as the local activity of methylating and demethylating enzymes. 